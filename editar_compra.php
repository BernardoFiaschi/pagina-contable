 ...
 * editar_compra.php
 * ----------------
 * Edición de una compra existente. Reusa la validación (leerComprobante) y el
 * formulario (comprobante_form.php) compartidos. Lo propio de "editar" es:
 *   - en GET: cargar la compra y sus líneas para precargar el formulario;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($errores) === 0) {
        try {
            ...
            $pdo->prepare("UPDATE compras SET
                empresa_id=:emp, proveedor_id=:prov, comprobante_tipo=:ct, comprobante_letra=:cl,
                ...
                WHERE id=:id")
                ->execute([
                    ':emp'=>$val['empresa_id'], ':prov'=>$val['contacto_id'], ':ct'=>$val['comprobante_tipo'],
            ...
            $pdo->prepare("DELETE FROM compra_alicuotas WHERE compra_id = :id")->execute([':id'=>$id]);
            ...
            $pdo->commit();
            header('Location: /compras.php?msg=' . urlencode('Compra actualizada.')); exit;
    ...
    $v = [
        'empresa_id'=>$_POST['empresa_id'] ?? '', 'contacto_id'=>$_POST['proveedor_id'] ?? '',
    ...
    // GET: cargar la compra y sus líneas desde la base para precargar el form.
    $st = $pdo->prepare("SELECT * FROM compras WHERE id = :id");
    $st->execute([':id' => $id]);
    $row = $st->fetch();
    if (!$row) { header('Location: /compras.php'); exit; }

    // Pasamos los nombres de columna a las claves que espera el form compartido.
    $v = $row;
    $v['contacto_id'] = $row['proveedor_id'];

    $stL = $pdo->prepare("SELECT alicuota, monto_gravado FROM compra_alicuotas WHERE compra_id = :id ORDER BY alicuota DESC");
    $stL->execute([':id' => $id]);
    $filas = $stL->fetchAll();
}

// Variables para el formulario compartido.
$accion       = '/editar_compra.php?id=' . $id;
$etiquetaCont = 'Proveedor';
$campoCont    = 'proveedor_id';
